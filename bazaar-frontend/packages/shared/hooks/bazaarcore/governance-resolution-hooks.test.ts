...
import {
  MOCK_SSU_ID,
  MOCK_SSU_GOV_ID,
  MOCK_SSU_GOV_CREATED_EVENT,
  MOCK_SSU_GOV_CREATED_EVENT_V2,
  MOCK_INCOMPLETE_EVENT,
  MOCK_TX_BLOCK_WITH_SSU_GOV,
  MOCK_TX_BLOCK_WITHOUT_SSU_GOV,
  MOCK_TX_BLOCK_PRUNED,
  MOCK_MULTIGET_TYPED,
  EXPECTED_SHARED_OBJECTS,
  MOCK_USER_STORAGE_ID,
  buildDummyEvents,
} from "../../__fixtures__/event-payloads";
...
const mockSuiClient = vi.hoisted(() => ({
  queryEvents: Object.assign((..._args: unknown[]) => Promise.resolve({ data: [] }), { mockResolvedValue: () => undefined, mockResolvedValueOnce: () => undefined, mockReset: () => undefined }),
  getTransactionBlock: Object.assign((..._args: unknown[]) => Promise.resolve({ objectChanges: [] }), { mockResolvedValue: () => undefined, mockResolvedValueOnce: () => undefined, mockReset: () => undefined }),
  getObject: Object.assign((..._args: unknown[]) => Promise.resolve({ data: {} }), { mockResolvedValue: () => undefined, mockResolvedValueOnce: () => undefined, mockReset: () => undefined }),
  multiGetObjects: Object.assign((..._args: unknown[]) => Promise.resolve([]), { mockResolvedValue: () => undefined, mockResolvedValueOnce: () => undefined, mockReset: () => undefined }),
}));
mockSuiClient.queryEvents = vi.fn();
mockSuiClient.getTransactionBlock = vi.fn();
mockSuiClient.getObject = vi.fn();
mockSuiClient.multiGetObjects = vi.fn();
...
function makeGovObjResponse(ssuId: string) {
  return {
    data: {
      content: {
        fields: { ssu_id: ssuId },
...
describe("useSSUSharedObjects", () => {
  ...
  it("UT-04: uses the first matching event when multiple events exist for same ssuId", async () => {
    ...
    expect(mockSuiClient.getTransactionBlock).toHaveBeenCalledWith(
      expect.objectContaining({ digest: MOCK_SSU_GOV_CREATED_EVENT.id.txDigest }),
    );
  });
  ...
  // GH-15: objectChanges pruned (null) — fall back to effects.created +
  // multiGetObjects(showType) and still resolve all 7 IDs. Regression for the
  // "No SSUGovernanceCreated event found" bug on aged bootstrap TXs (2026-06-24).
  it("GH-15: resolves via effects.created fallback when objectChanges is pruned", async () => {
    mockSuiClient.queryEvents.mockResolvedValue({
      data: [MOCK_SSU_GOV_CREATED_EVENT],
      nextCursor: null,
      hasNextPage: false,
    });
    // Fullnode pruned objectChanges (absent) but effects.created survives.
    mockSuiClient.getTransactionBlock.mockResolvedValue(MOCK_TX_BLOCK_PRUNED);
    mockSuiClient.multiGetObjects.mockResolvedValue(MOCK_MULTIGET_TYPED);
    mockSuiClient.getObject.mockResolvedValue(makeGovObjResponse(MOCK_SSU_ID));
    ...
    expect(result.current.data).toEqual(EXPECTED_SHARED_OBJECTS);
    // Fallback path was exercised (objectChanges absent → multiGetObjects called).
    expect(mockSuiClient.multiGetObjects).toHaveBeenCalledOnce();
  });
...
describe("_findSSUGovernanceCreatedEvent (exercised via useSSUGovId)", () => {
  ...
  it("GH-12: helper issues independent queryEvents when called from two hooks", async () => {
    ...
    const { result: rGov } = renderHook(
      () => useSSUGovId(MOCK_SSU_ID),
      { wrapper: WrapperA },
    );
    const { result: rShared } = renderHook(
      () => useSSUSharedObjects(MOCK_SSU_ID),
      { wrapper: WrapperB },
    );
  ...
  // GH-16: useSSUGovId also falls back to effects.created + multiGetObjects when
  // objectChanges is pruned to null (companion to GH-15 for the gov-id-only hook).
  it("GH-16: useSSUGovId resolves via effects.created fallback when objectChanges pruned", async () => {
    mockSuiClient.queryEvents.mockResolvedValue({
      data: [MOCK_SSU_GOV_CREATED_EVENT],
      nextCursor: null,
      hasNextPage: false,
