...
function makeMemberRegistryObj(tableUid: string) {
  return {
    data: {
      content: {
        fields: {
...
function makeDynamicFieldsPage(entries: Array<{ name: { type: string; value: unknown }; objectId?: string }>) {
  return { data: entries, nextCursor: null, hasNextPage: false };
}

function makeMemberEntryObj(player: string, isBanned: boolean, registeredAtMs: number) {
  return {
    data: {
      content: {
        fields: {
...
describe("useBanList", () => {
  ...
  it("BL-02: filters to only is_banned=true entries and populates expiryMs from registered_at_ms", async () => {
    ...
    mockSuiClient.getDynamicFieldObject
      .mockResolvedValueOnce(makeMemberEntryObj(PLAYER_BANNED, true,  1_700_000_000_000))
      .mockResolvedValueOnce(makeMemberEntryObj(PLAYER_ACTIVE, false, 1_699_000_000_000));
    ...
    expect(entry.expiryMs).toBe(1_700_000_000_000);

    expect(mockSuiClient.getObject).toHaveBeenCalledWith(
      expect.objectContaining({ id: MOCK_MEMBER_REG_ID }),
    );
    expect(mockSuiClient.getDynamicFields).toHaveBeenCalledWith(
      expect.objectContaining({ parentId: MOCK_TABLE_UID, limit: 200 }),
    );
