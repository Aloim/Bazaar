...
function makeTribeGovWithWidgetConfig(widgetConfigId) {
    return {
        data: {
            content: {
                fields: {
...
describe("useTribeWidgetConfigId", () => {
    ...
    it("T01 — returns null immediately when tribeGovId is null (query disabled)", async () => {
        ...
        const { result } = renderHook(() => useTribeWidgetConfigId(null), { wrapper: Wrapper });
    ...
    it("T02 — returns widgetConfigId string when Option<ID> is populated", async () => {
        mockSuiClient.getObject.mockResolvedValueOnce(makeTribeGovWithWidgetConfig(WIDGET_CFG_ID));
        const { Wrapper } = makeWrapper();
        const { result } = renderHook(() => useTribeWidgetConfigId(TRIBE_GOV_ID), { wrapper: Wrapper });
    ...
    it("T03 — returns null when Option<ID> is empty (legacy v4 tribe without widget_config_id)", async () => {
        mockSuiClient.getObject.mockResolvedValueOnce(makeTribeGovWithoutWidgetConfig());
        const { Wrapper } = makeWrapper();
        const { result } = renderHook(() => useTribeWidgetConfigId(TRIBE_GOV_ID), { wrapper: Wrapper });
        await waitFor(() => expect(result.current.isLoading).toBe(false));
        expect(result.current.data).toBeNull();
