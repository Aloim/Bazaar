...
function makeUserStorageObj(tableUid: string) {
  return {
    data: {
      content: {
        fields: {
...
function makeDFPage(entries: Array<{ name: { type: string; value: unknown } }>) {
  return { data: entries, nextCursor: null, hasNextPage: false };
}

function makeUnclaimedItemObj(owner: string, typeId: number, qty: number, shopId: string, expiryMs: number) {
  return {
    data: {
      content: {
        fields: {
...
function makeSharedObjs(userStorageId: string) {
  return {
    data: {
      ssuGovId:            "0xgov",
...
describe("useUnclaimedItems", () => {
  ...
  it("UI-02: filters unclaimed items by original_owner === walletAddress", async () => {
    ...
    mockSuiClient.getDynamicFieldObject
      .mockResolvedValueOnce(makeUnclaimedItemObj(WALLET_ADDRESS,  42, 10, "0xshop1", 1_800_000_000_000))
      .mockResolvedValueOnce(makeUnclaimedItemObj(OTHER_ADDRESS,   99,  5, "0xshop2", 1_700_000_000_000));
    ...
    expect(item.expiryMs).toBe(1_800_000_000_000);
    expect(result.current.error).toBeNull();
  });
  ...
  it("UI-03: polling refetch fires after 30s and re-fetches items", async () => {
    ...
    // First load: 1 item.
    mockSuiClient.getObject.mockResolvedValue(makeUserStorageObj(MOCK_TABLE_UID));
    mockSuiClient.getDynamicFields.mockResolvedValue(
      makeDFPage([{ name: { type: "0x2::object::ID", value: ITEM_ID_1 } }]),
    );
    mockSuiClient.getDynamicFieldObject.mockResolvedValue(
      makeUnclaimedItemObj(WALLET_ADDRESS, 42, 10, "0xshop1", 1_800_000_000_000),
    );
