...
const mockFetchBoard = vi.hoisted(() => vi.fn<(boardId: string) => Promise<import("../announcement-board-fetch").AnnouncementData[]>>());
...
function makeTribeGovObj(ssuIds: string[]) {
  return {
    data: {
      content: {
...
function makeSsuGovObj(boardId: string | null) {
  return {
    data: {
      content: {
...
function makeAnn(
  id: number,
  opts: { isSticky?: boolean; createdAtMs?: number; ssuHint?: string } = {},
): import("../announcement-board-fetch").AnnouncementData {
  return {
    id,
    author:      "0xauthor",
    title:       `Title ${id}`,
...
describe("useTribeAnnouncements", () => {
  ...
  it("T02: returns undefined (query disabled) when tribe has 0 SSUs", async () => {
    ...
    await waitFor(() => !result.current.isPending || result.current.data !== undefined, { timeout: 2000 })
      .catch(() => { /* disabled inner query never resolves — that is correct */ });
