...
function setup(chat: string) {
  const sendToGodot = vi.fn();
  const playerPositionRef = createRef<PlayerPosition | null>() as {
...
describe("useMultiplayerRelay — chat piggyback", () => {
  it("RL-01: outbound position message includes chatTextRef text", () => {
    ...
    const msgs = (lastSocket as FakeWebSocket).sent.map((s) => JSON.parse(s));
  ...
  it("RL-02: inbound position.chat flows into the PLAYER_POSITIONS payload", () => {
    ...
    act(() => {
      (lastSocket as FakeWebSocket)._message({
        type: "position",
        address: REMOTE,
        displayName: "Them",
    ...
    const players = call![1].players as Array<{ address: string; chat: string }>;
...
describe("useMultiplayerRelay — relationship colour (own/tribe/other)", () => {
  function remotePlayer(sendToGodot: ReturnType<typeof vi.fn>) {
    const call = sendToGodot.mock.calls.filter((c) => c[0] === "PLAYER_POSITIONS").pop();
    const players = call![1].players as Array<{ address: string; tier: string; color: string }>;
  ...
  it("same SSU → own (yellow/orange)", () => {
    const { sendToGodot } = setup("");
    act(() => { lastSocket?._open(); });
    act(() => { (lastSocket as FakeWebSocket)._message({
  ...
  it("outbound position carries self ssuId + bazaarType", () => {
    ...
    const pos = (lastSocket as FakeWebSocket).sent.map((s) => JSON.parse(s)).find((m) => m.type === "position");
...
describe("useMultiplayerRelay — committed-message history piggyback", () => {
  it("RL-03: a peer msgSeq increment fires onRemoteMessage (first sighting skipped, no dupes)", () => {
    ...
    const playerPositionRef = { current: { x: 0, y: 0, direction: "se" } } as {
      current: PlayerPosition | null;
    };
    renderHook(() =>
      useMultiplayerRelay({
...
describe("useMultiplayerRelay — double-login (active session wins)", () => {
  ...
  function lastPlayers(sendToGodot: ReturnType<typeof vi.fn>) {
    const call = sendToGodot.mock.calls.filter((c) => c[0] === "PLAYER_POSITIONS").pop();
    return (call![1].players as Array<{ address: string; x: number }>);
  }
  const remoteX = (s: ReturnType<typeof vi.fn>) => lastPlayers(s).find((p) => p.address === REMOTE)?.x;
  ...
  it("DL-03: only the owning session's leave clears the avatar", () => {
    ...
    act(() => { (lastSocket as FakeWebSocket)._message({ type: "leave", address: REMOTE, sessionId: "B" }); });
    expect(lastPlayers(sendToGodot).length).toBe(1);
    // Owner A leaves → avatar removed.
    act(() => { (lastSocket as FakeWebSocket)._message({ type: "leave", address: REMOTE, sessionId: "A" }); });
    expect(lastPlayers(sendToGodot).length).toBe(0);
  });
});

describe("useMultiplayerRelay — identity binding (AUD-UX-02 / B5)", () => {
  function setupWithSign(signFn?: (msg: string) => Promise<{ signature: string }>) {
    const sendToGodot = vi.fn();
    const playerPositionRef = { current: { x: 1, y: 1, direction: "se" } } as {
      current: PlayerPosition | null;
    };
    renderHook(() =>
      useMultiplayerRelay({
  ...
  const sentTypes = () =>
    (lastSocket as FakeWebSocket).sent.map((s) => JSON.parse(s) as { type: string });
  ...
  it("IB-02: auth_challenge → auth_response carries nonce + signature, no publicKey", async () => {
    ...
    await act(async () => {
      (lastSocket as FakeWebSocket)._message({ type: "auth_challenge", nonce: "deadbeef1234" });
      await Promise.resolve(); await Promise.resolve();
    });
    const resp = (lastSocket as FakeWebSocket).sent
  ...
  it("IB-03: on auth_ok the send loop starts and position messages flow", async () => {
    setupWithSign(vi.fn().mockResolvedValue({ signature: "sig_abc" }));
    await act(async () => {
      (lastSocket as FakeWebSocket)._message({ type: "auth_challenge", nonce: "abc" });
      await Promise.resolve(); await Promise.resolve();
    });
    act(() => { (lastSocket as FakeWebSocket)._message({ type: "auth_ok", verifiedAddress: LOCAL }); });
  ...
  it("IB-05: auth_fail logs a warning and still starts the send loop (graceful degradation)", () => {
    ...
    act(() => { (lastSocket as FakeWebSocket)._message({ type: "auth_fail", reason: "invalid_signature" }); });
