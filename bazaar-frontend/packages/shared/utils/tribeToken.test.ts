...
describe("formatTribeAmount", () => {
  ...
  it("applies en-US thousands separator by default", () => {
    ...
    expect(formatTribeAmount(10_000_000n)).toBe("100,000.00");
    expect(formatTribeAmount(12_345_678n)).toBe("123,456.78");
  });
  ...
  it("honors noGrouping for re-injection into input fields", () => {
    ...
    expect(formatTribeAmount(12_345_678n, { noGrouping: true })).toBe("123456.78");
  });
...
describe("parseTribeAmount — accepting cases", () => {
  it("parses integer strings", () => {
    ...
    expect(parseTribeAmount("1000")).toBe(100_000n);
  });
  ...
  it("strips US-style thousands separators", () => {
    expect(parseTribeAmount("1,000")).toBe(100_000n);
    expect(parseTribeAmount("1,234.56")).toBe(123_456n);
    expect(parseTribeAmount("100,000.00")).toBe(10_000_000n);
  });
...
describe("parseTribeAmountSafe", () => {
  ...
  it("returns reason on failure (no throw)", () => {
    ...
    if (!r1.ok) expect(r1.reason).toMatch(/max 2 decimal places/);

    const r2 = parseTribeAmountSafe("-1");
    expect(r2.ok).toBe(false);
    if (!r2.ok) expect(r2.reason).toMatch(/negative/);

    const r3 = parseTribeAmountSafe("");
    expect(r3.ok).toBe(false);
    if (!r3.ok) expect(r3.reason).toMatch(/empty/);
  });
});
...
describe("format/parse round-trip", () => {
  ...
  it("parse(format(n)) returns the same scaled-unit bigint", () => {
    for (const scaled of [0n, 1n, 99n, 100n, 150n, 12345n, 10_000_000n]) {
