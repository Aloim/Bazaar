...
describe("computeTaxBreakdownBps — single-step floor semantics", () => {
    it("computes the total with one floor over the summed bps", () => {
        const b = computeTaxBreakdownBps(1000000, 525, 200, 200);
        expect(b.totalBps).toBe(925);
        expect(b.totalAmount).toBe(Math.floor((1000000 * 925) / 10000));
        expect(b.net).toBe(1000000 - b.totalAmount);
    });
    ...
    it("zero gross / zero bps stay zero", () => {
        ...
        expect(computeTaxBreakdownBps(1000000, 0, 0, 0).totalAmount).toBe(0);
    });
});
describe("computeTaxBreakdownFlat — DE carve semantics (AUD-NT-16/ET-03)", () => {
    it("payer total is the gross flats — the DApp carve never inflates it", () => {
        const f = computeTaxBreakdownFlat(2, 1000000, 500000, 200);
        expect(f.totalAmount).toBe(2 * (1000000 + 500000));
    ...
    it("NoTribe DE: DApp share carved from the SSU flat (the AUD-NT-16 catch)", () => {
        ...
        const f = computeTaxBreakdownFlat(1, 1000000000, 0, 200);
        expect(f.dappAmount).toBe(20000000); // NOT 0 — the old FE hid this
        expect(f.ssuAmount).toBe(980000000); // NOT the full flat — net of carve
        expect(f.tribeAmount).toBe(0);
        expect(f.totalAmount).toBe(1000000000); // payer pays the flat, unchanged
    });
    it("Easy DE: per-layer floors (mirrors the post-V36 ET-02 rounding fix)", () => {
        ...
        const dappFromSsu = Math.floor((ssu * bps) / 10000);
        const dappFromTribe = Math.floor((tribe * bps) / 10000);
    ...
    it("Easy DE payment sizing includes the tribe flat (the AUD-ET-01 catch)", () => {
        ...
        const f = computeTaxBreakdownFlat(3, 200000, 50000, 0);
        expect(f.totalAmount).toBe(3 * 250000);
        expect(f.totalAmount).toBeGreaterThan(3 * 200000);
    });
