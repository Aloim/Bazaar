...
import { classifyShopTier, type ShopTierContext, type ShopTierInput } from "./shopTier";

const OWN_SSU = "0xown";
const OTHER_SSU = "0xother";

function shop(p: Partial<ShopTierInput>): ShopTierInput {
  return { ssuId: OTHER_SSU, bazaarType: 0, tribeId: 0, ...p };
}

describe("classifyShopTier — OWN precedence", () => {
  const ctx: ShopTierContext = { currentSsuId: OWN_SSU, currentBazaarType: "Advanced", currentTribeId: 7 };
...
describe("classifyShopTier — Easy/Advanced context", () => {
  const ctx: ShopTierContext = { currentSsuId: OWN_SSU, currentBazaarType: "Easy", currentTribeId: 42 };
...
describe("classifyShopTier — edge cases", () => {
  it("never classifies tribeId=0 as 'tribe' under Easy/Advanced", () => {
    const ctx: ShopTierContext = { currentSsuId: OWN_SSU, currentBazaarType: "Advanced", currentTribeId: 0 };
  ...
  it("falls back to 'other' (never wrong 'tribe') while context type is null/loading", () => {
    const ctx: ShopTierContext = { currentSsuId: OWN_SSU, currentBazaarType: null, currentTribeId: null };
