...
import { composeMissionFee, missionRewardTotals, validateMissionDraft, missionCountdown, MAX_PROOF_ITEMS, MAX_REWARD_ITEMS, MAX_DURATION_HOURS, MS_PER_HOUR, VISIBILITY, composeVisibilityMask, missionVisibleToRole, viewerRoleMask, missionCollateralRaw, } from "./mission";
...
describe("missionRewardTotals", () => {
    it("scales human money to raw per-run and multiplies by runs", () => {
        const r = missionRewardTotals({ rewardMoney: 1.5, priceScale: 1000000000, maxRuns: 3 });
        expect(r.rewardPerRunRaw).toBe(1500000000);
        expect(r.rewardTotalRaw).toBe(4500000000);
    });
    it("rounds the scaled per-run amount", () => {
        const r = missionRewardTotals({ rewardMoney: 0.0000000015, priceScale: 1000000000, maxRuns: 2 });
        expect(r.rewardPerRunRaw).toBe(2); // round(1.5) = 2
        expect(r.rewardTotalRaw).toBe(4);
    });
    it("zero money → zero totals (no item-less escrow)", () => {
        const r = missionRewardTotals({ rewardMoney: 0, priceScale: 1000000000, maxRuns: 5 });
...
describe("missionCountdown", () => {
    ...
    it("hours remaining → 'Nh Nm'", () => {
        const r = missionCountdown(NOW + 4 * MS_PER_HOUR + 12 * 60000, NOW);
        expect(r.label).toBe("4h 12m");
    });
    it("minutes only → 'Nm'", () => {
        const r = missionCountdown(NOW + 8 * 60000, NOW);
...
describe("missionCollateralRaw", () => {
    it("mirrors missionRewardTotals scale", () => {
        expect(missionCollateralRaw({ collateralMoney: 1.5, priceScale: 1000000000 })).toBe(1500000000);
    });
    it("zero input → 0", () => {
        expect(missionCollateralRaw({ collateralMoney: 0, priceScale: 1000000000 })).toBe(0);
    });
});
...
describe("parseAcceptance collateralAmount", () => {
    ...
    it("defaults to 0 when field absent (V33 acceptances)", () => {
        ...
        expect(Number(raw.collateral_amount ?? 0)).toBe(0);
    });
});
// ============================================================
