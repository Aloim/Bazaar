...
import { RATE_PRECISION, MIN_SWAP_EVE_MIST, availableLiquidity, scaledRateV28, quoteEveToTokensV28, quoteEveCostForTokensV28, quoteTokensToEveV28, quoteLegacyRedeemEveOut, } from "./exchangeQuotes";
// 1000 EVE vault, no reserve, tribe wallet holds 500 token units.
const POOL = {
    eveBalance: 1000000000000n,
    reserveMist: 0n,
...
describe("availableLiquidity / scaledRateV28", () => {
    ...
    it("scaled rate = available * PRECISION / tribeWalletBalance", () => {
        expect(scaledRateV28(POOL)).toBe((1000000000000n * RATE_PRECISION) / 500n);
    });
...
describe("quoteEveToTokensV28 (buy — mirrors swap_eve_to_tokens)", () => {
    it("computes floor((gross - tax) * walletBal / available)", () => {
        ...
        expect(quoteEveToTokensV28(POOL, 10000000000n, 500)).toBe(4n);
    });
    it("uses the TRIBE WALLET stock as denominator, not total supply", () => {
        ...
        expect(quoteEveToTokensV28(doubled, 10000000000n, 500)).toBe(9n); // 9.5 → 9
    });
    it("returns 0n where Move aborts: output exceeding wallet stock", () => {
        ...
        expect(quoteEveToTokensV28(tiny, 2000000000000n, 0)).toBe(0n);
    });
    it("returns 0n on zero/eaten input and dead pools", () => {
        expect(quoteEveToTokensV28(POOL, 0n, 500)).toBe(0n);
        expect(quoteEveToTokensV28({ ...POOL, tribeWalletBalance: 0n }, 10000n, 0)).toBe(0n);
        expect(quoteEveToTokensV28({ ...POOL, eveBalance: 0n }, 10000n, 0)).toBe(0n);
        expect(quoteEveToTokensV28(POOL, 10n, 10000)).toBe(0n); // fee eats everything
    });
});
describe("quoteEveCostForTokensV28 (inverse buy)", () => {
    ...
    it("clamps up to the Move min-swap floor", () => {
        ...
        const dust = { eveBalance: 1000000n, reserveMist: 0n, tribeWalletBalance: 1000n };
...
describe("quoteTokensToEveV28 (sell — mirrors swap_tokens_to_eve)", () => {
    it("computes gross = tokens * scaledRate / PRECISION, minus fee", () => {
        ...
        expect(quoteTokensToEveV28(POOL, 5n, 500)).toBe(10000000000n - 500000000n);
    });
...
describe("quoteLegacyRedeemEveOut (mirrors withdraw_ssu_tax_credits)", () => {
    it("uses TOTAL SUPPLY as denominator — diverges from the V28 swap rate", () => {
        ...
        const eveBalance = 1000000000000n;
        const legacy = quoteLegacyRedeemEveOut({ eveBalance, reserveMist: 0n, totalSupply: 10000n }, 100n);
        // legacy: 100 * (1e12 * 1e9 / 1e4) / 1e9 = 1e10 (10 EVE)
        expect(legacy).toBe(10000000000n);
        // V28 swap of the same 100 tokens (wallet stock 500) pays 20x more —
        // the ADV-07 divergence the redeem UI must warn about.
        const v28 = quoteTokensToEveV28({ eveBalance, reserveMist: 0n, tribeWalletBalance: 500n }, 100n, 0);
        expect(v28).toBe(200000000000n);
        expect(v28).not.toBe(legacy);
    });
    it("enforces the reserve floor", () => {
        expect(quoteLegacyRedeemEveOut({ eveBalance: 1000000n, reserveMist: 999999n, totalSupply: 10n }, 5n)).toBe(0n);
    });
