...
describe("isExtensionConfigFrozenError", () => {
    it("matches the literal world abort name (the real wallet message)", () => {
        const e = new Error('code 32603 message "Transaction Resolution failed" MoveAbort in 5th command, ' +
    ...
    it("matches case-insensitively and on the truncated module form", () => {
        ...
        expect(isExtensionConfigFrozenError("Move abort: frozen in 0xabc::storage_unit::authorize")).toBe(true);
    });
    ...
    it("does NOT match the Bazaar's own E_SSU_FROZEN commerce freeze", () => {
        ...
        expect(isExtensionConfigFrozenError(new Error("MoveAbort E_SSU_FROZEN in ssu_governance::assert_not_frozen"))).toBe(false);
    });
